...
static inline CFStringRef MakeCFString(const char* cStr) { return CFStringCreateWithCString(0, cStr, kCFStringEncodingUTF8); }
  ...
  case kAudioUnitProperty_ParameterInfo: // 4,  listenable
  {
    ...
    *pDataSize = sizeof(AudioUnitParameterInfo);
    if (pData)
    {
      ...
      pInfo->flags = kAudioUnitParameterFlag_CFNameRelease | kAudioUnitParameterFlag_HasCFNameString | kAudioUnitParameterFlag_IsWritable | kAudioUnitParameterFlag_IsReadable;
      ...
      default: {
        ...
        case IParam::kUnitSamples:
          pInfo->unit = kAudioUnitParameterUnit_SampleFrames;
          break;
        case IParam::kUnitDB:
          pInfo->unit = kAudioUnitParameterUnit_Decibels;
          break;
        case IParam::kUnitLinearGain:
          pInfo->unit = kAudioUnitParameterUnit_LinearGain;
          break;
        case IParam::kUnitPan:
          pInfo->unit = kAudioUnitParameterUnit_Pan;
          break;
        case IParam::kUnitPhase:
          pInfo->unit = kAudioUnitParameterUnit_Phase;
          break;
        case IParam::kUnitDegrees:
          pInfo->unit = kAudioUnitParameterUnit_Degrees;
          break;
        case IParam::kUnitMeters:
          pInfo->unit = kAudioUnitParameterUnit_Meters;
          break;
        case IParam::kUnitRate:
          pInfo->unit = kAudioUnitParameterUnit_Rate;
          break;
        case IParam::kUnitRatio:
          pInfo->unit = kAudioUnitParameterUnit_Ratio;
          break;
        case IParam::kUnitFrequency:
          pInfo->unit = kAudioUnitParameterUnit_Hertz;
          break;
        case IParam::kUnitOctaves:
          pInfo->unit = kAudioUnitParameterUnit_Octaves;
          break;
        case IParam::kUnitCents:
          pInfo->unit = kAudioUnitParameterUnit_Cents;
          break;
        case IParam::kUnitAbsCents:
          pInfo->unit = kAudioUnitParameterUnit_AbsoluteCents;
          break;
        case IParam::kUnitSemitones:
          pInfo->unit = kAudioUnitParameterUnit_RelativeSemiTones;
          break;
        case IParam::kUnitMIDINote:
          pInfo->unit = kAudioUnitParameterUnit_MIDINoteNumber;
          break;
        case IParam::kUnitMIDICtrlNum:
          pInfo->unit = kAudioUnitParameterUnit_MIDIController;
          break;
        case IParam::kUnitBPM:
          pInfo->unit = kAudioUnitParameterUnit_BPM;
          break;
        case IParam::kUnitBeats:
          pInfo->unit = kAudioUnitParameterUnit_Beats;
          break;
...
OSStatus IPlugAU::SetProperty(AudioUnitPropertyID propID, AudioUnitScope scope, AudioUnitElement element, UInt32* pDataSize, const void* pData)
{
  ...
  case kAudioUnitProperty_MakeConnection: // 1,
  {
    ...
    if (pAUC->sourceAudioUnit) // Opening connection.
    {
      ...
      negotiatedOK = // Ask whoever is sending us audio what the format is.
        (AudioUnitGetProperty(pAUC->sourceAudioUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Output, pAUC->sourceOutputNumber, &srcASBD, &size) == noErr);
      negotiatedOK &= // Try to set our own format to match.
        (SetProperty(kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, pAUC->destInputNumber, &size, &srcASBD) == noErr);
      if (negotiatedOK) // Connection terms successfully negotiated.
      {
        ...
        if (AudioUnitGetProperty(pAUC->sourceAudioUnit, kAudioUnitProperty_FastDispatch, kAudioUnitScope_Global, kAudioUnitRenderSelect, &srcRenderProc, &size) == noErr)
        {
  ...
  case kAudioUnitProperty_AUHostIdentifier:             // 46,
  {
    if (GetHost() == kHostUninit)
    {
      ...
      int version = (pHostID->hostVersion.majorRev << 16) + ((pHostID->hostVersion.minorAndBugRev & 0xF0) << 4) + ((pHostID->hostVersion.minorAndBugRev & 0x0F));
...
const char* IPlugAU::AUInputTypeStr(int type)
{
  ...
  {
  case IPlugAU::eDirectFastProc:
    return "DirectFastProc";
  case IPlugAU::eDirectNoFastProc:
    return "DirectNoFastProc";
  case IPlugAU::eRenderCallback:
...
OSStatus IPlugAU::SetState(CFPropertyListRef pPropList)
{
  ...
  char presetName[64];
  if (!GetNumberFromDict(pDict, kAUPresetVersionKey, &version, kCFNumberSInt32Type) || !GetNumberFromDict(pDict, kAUPresetTypeKey, &type, kCFNumberSInt32Type)
      || !GetNumberFromDict(pDict, kAUPresetSubtypeKey, &subtype, kCFNumberSInt32Type) || !GetNumberFromDict(pDict, kAUPresetManufacturerKey, &mfr, kCFNumberSInt32Type)
...
static IPlugAU* GetPlug(void* x) { return (IPlugAU*)&((AudioComponentPlugInInstance*)x)->mInstanceStorage; }

// static
OSStatus IPlugAU::AUMethodInitialize(void* pSelf) { return DoInitialize(GetPlug(pSelf)); }

// static
OSStatus IPlugAU::AUMethodUninitialize(void* pSelf) { return DoUninitialize(GetPlug(pSelf)); }
...
// static
OSStatus IPlugAU::AUMethodRemovePropertyListener(void* pSelf, AudioUnitPropertyID prop, AudioUnitPropertyListenerProc proc) { return DoRemovePropertyListener(GetPlug(pSelf), prop, proc); }
...
// static
OSStatus IPlugAU::AUMethodAddRenderNotify(void* pSelf, AURenderCallback proc, void* userData) { return DoAddRenderNotify(GetPlug(pSelf), proc, userData); }

// static
OSStatus IPlugAU::AUMethodRemoveRenderNotify(void* pSelf, AURenderCallback proc, void* userData) { return DoRemoveRenderNotify(GetPlug(pSelf), proc, userData); }
...
// static
OSStatus IPlugAU::AUMethodScheduleParameters(void* pSelf, const AudioUnitParameterEvent* pEvent, UInt32 nEvents) { return DoScheduleParameters(GetPlug(pSelf), pEvent, nEvents); }
...
// static
OSStatus IPlugAU::AUMethodReset(void* pSelf, AudioUnitScope scope, AudioUnitElement elem) { return DoReset(GetPlug(pSelf)); }
...
// static
OSStatus IPlugAU::AUMethodSysEx(void* pSelf, const UInt8* inData, UInt32 inLength) { return DoSysEx(GetPlug(pSelf), inData, inLength); }
#endif
...
OSStatus IPlugAU::DoRemovePropertyListenerWithUserData(IPlugAU* _this, AudioUnitPropertyID prop, AudioUnitPropertyListenerProc proc, void* userData)
{
  ...
  {
    PropertyListener* pListener = _this->mPropertyListeners.Get(i);
    if (listener.mPropID == pListener->mPropID && listener.mListenerProc == pListener->mListenerProc && listener.mProcArgs == pListener->mProcArgs)
    {
      _this->mPropertyListeners.Delete(i, true);
      break;
    }
...
OSStatus IPlugAU::DoScheduleParameters(IPlugAU* _this, const AudioUnitParameterEvent* pEvent, UInt32 nEvents)
{
  ...
  for (int i = 0; i < nEvents; ++i, ++pEvent)
  {
    ...
    {
      OSStatus r = SetParamProc(_this, pEvent->parameter, pEvent->scope, pEvent->element, pEvent->eventValues.immediate.value, pEvent->eventValues.immediate.bufferOffset);
      if (r != noErr)
      {
        return r;
      }
