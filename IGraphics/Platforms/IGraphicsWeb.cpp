...
static EM_BOOL key_callback(int eventType, const EmscriptenKeyboardEvent* pEvent, void* pUserData)
{
  ...
  IKeyPress keyPress{keyUTF8.Get(), DOMKeyToVirtualKey(pEvent->keyCode), static_cast<bool>(pEvent->shiftKey), static_cast<bool>(pEvent->ctrlKey || pEvent->metaKey), static_cast<bool>(pEvent->altKey)};
...
static EM_BOOL wheel_callback(int eventType, const EmscriptenWheelEvent* pEvent, void* pUserData)
{
  ...
  case EMSCRIPTEN_EVENT_WHEEL:
    pGraphics->OnMouseWheel(x, y, modifiers, pEvent->deltaY);
  default:
    break;
  }
...
EM_BOOL touch_callback(int eventType, const EmscriptenTouchEvent* pEvent, void* pUserData)
{
  ...
  for (auto i = 0; i < pEvent->numTouches; i++)
  {
    ...
    info.ms = {true, false, static_cast<bool>(pEvent->shiftKey), static_cast<bool>(pEvent->ctrlKey), static_cast<bool>(pEvent->altKey), static_cast<ITouchID>(pEvent->touches[i].identifier)};
...
static EM_BOOL text_entry_keydown(int eventType, const EmscriptenKeyboardEvent* pEvent, void* pUserData)
{
  ...
  IKeyPress keyPress{pEvent->key, DOMKeyToVirtualKey(pEvent->keyCode), static_cast<bool>(pEvent->shiftKey), static_cast<bool>(pEvent->ctrlKey), static_cast<bool>(pEvent->altKey)};
...
IPopupMenu* IGraphicsWeb::CreatePlatformPopupMenu(IPopupMenu& menu, const IRECT bounds, bool& isAsync) { return nullptr; }
...
PlatformFontPtr IGraphicsWeb::LoadPlatformFont(const char* fontID, void* pData, int dataSize) { return PlatformFontPtr(new MemoryFont(fontID, "", pData, dataSize)); }
