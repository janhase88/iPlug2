...
| `DBGMSG` / `IGRAPHICS_VK_LOG` (event: `PlatformResize`) | Logged on each platform resize, showing the window (host) scale, render (physical) scale, chosen target scale, bypass flag, virtualization ratio, and whether the parent already resized.【F:IGraphics/Platforms/IGraphicsWin.cpp†L3526-L3576】 | Use to confirm the window stays at the host DPI (`targetScale`≈host) while the virtualization ratio reflects the physical pixels the renderer targets. |
