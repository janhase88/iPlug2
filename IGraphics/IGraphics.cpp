...
IGraphics::IGraphics(IGEditorDelegate& dlg, int w, int h, int fps, float scale)
  ...
  , mDelegate(&dlg)
{
  StaticStorage<APIBitmap>::Accessor bitmapStorage(
#if IPLUG_SEPARATE_BITMAP_CACHE
    mBitmapCache
#else
    sBitmapCache
  ...
  StaticStorage<SVGHolder>::Accessor svgStorage(
#if IPLUG_SEPARATE_SVG_CACHE
    mSVGCache
#else
    sSVGCache
...
{
  StaticStorage<SVGHolder>::Accessor storage(
  #if IPLUG_SEPARATE_SVG_CACHE
    mSVGCache
  #else
    sSVGCache
...
IBitmap IGraphics::LoadBitmap(const char* name, int nStates, bool framesAreHorizontal, int targetScale)
{
  ...
  StaticStorage<APIBitmap>::Accessor storage(
#if IPLUG_SEPARATE_BITMAP_CACHE
    mBitmapCache
#else
    sBitmapCache
...
EResourceLocation IGraphics::SearchImageResource(const char* name, const char* type, WDL_String& result, int targetScale, int& sourceScale)
{
  for (sourceScale = targetScale; sourceScale > 0; SearchNextScale(sourceScale, targetScale))
  {
    ...
    EResourceLocation found = EResourceLocation::kNotFound;

#if defined OS_IOS && defined IGRAPHICS_METAL && IPLUG_SEPARATE_IOS_TEXTURE_CACHE
    if (auto* pIOS = dynamic_cast<IGraphicsIOS*>(this))
    {
      auto& textureMap = pIOS->GetTextureMap();
      if (textureMap.find(fullName.Get()) != textureMap.end())
      {
        result.Set(fullName.Get());
        return EResourceLocation::kPreloadedTexture;
      }
    }
#endif

    found = LocateResource(fullName.Get(), type, result, GetBundleID(), GetWinModuleHandle(), GetSharedResourcesSubPath());
