  ...
  @autoreleasepool
  {
    ...
    if (isCorrectType && pFile)
    {
      ...
      if (subfolder)
      {
        ...
        NSString* pPath =
          [[[[pMusicLocation stringByAppendingPathComponent:pPluginName] stringByAppendingPathComponent:@"Resources"] stringByAppendingPathComponent:pFile] stringByAppendingPathExtension:pExt];
...
bool IsXPCAuHost() { return ([[[NSProcessInfo processInfo] processName] containsString:@"XPC"]); }

bool IsOOPAuv3AppExtension() { return ([[[NSBundle mainBundle] bundleIdentifier] containsString:@"AUv3"]); }
...
void VST3PresetsPath(WDL_String& path, const char* mfrName, const char* pluginName, bool isSystem) { path.Set(""); }

void INIPath(WDL_String& path, const char* pluginName) { path.Set(""); }
...
bool AppIsSandboxed() { return true; }
