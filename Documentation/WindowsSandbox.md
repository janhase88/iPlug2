...
- The core `IGraphics` bitmap/SVG caches now promote to renderer-owned storage whenever the draw umbrella or its child toggles are enabled, keeping shared statics only when every draw sandbox switch is disabled.【F:IGraphics/IGraphics.cpp†L43-L103】【F:IGraphics/IGraphics.h†L1873-L1876】
- Skia's Vulkan context state follows `IGRAPHICS_SANDBOX_VK_CONTEXT`, binding command pools, swapchain images, synchronization primitives, and logging metadata to `SkiaVulkanContextState` for sandbox builds while retaining the legacy shared singleton when the toggle is disabled.【F:IGraphics/Drawing/IGraphicsSkia.h†L259-L339】【F:IGraphics/Drawing/IGraphicsSkia.cpp†L250-L309】
