...
#if IPLUG_SEPARATE_LOGGER_STATE
extern IPlugLogger* gLogger;
inline IPlugLogger& GetLogger() { return *gLogger; }
#else
inline IPlugLogger& GetLogger()
{
  static IPlugLogger sLogger;
  return sLogger;
}
#endif
...
void Trace(const char* funcName, int line, const char* format, ...)
{
  ...
  if (logger.mTrace++ < MAX_LOG_LINES)
  {
  ...
  #else
    WDL_MutexLock lock(&logger.mLogMutex);
    ...
    {
      if (++logger.mProcessCount > MAX_PROCESS_TRACE_COUNT)
      {
        fflush(sLogFile->mFP);
        return;
      }
      else if (logger.mProcessCount == MAX_PROCESS_TRACE_COUNT)
      {
        fprintf(sLogFile->mFP, "**************** DISABLING PROCESS TRACING AFTER %d HITS ****************\n\n", logger.mProcessCount);
        fflush(sLogFile->mFP);
        return;
      }
    }
    ...
    #endif
    {
      if (++logger.mIdleCount > MAX_IDLE_TRACE_COUNT)
      {
        fflush(sLogFile->mFP);
        return;
      }
      else if (logger.mIdleCount == MAX_IDLE_TRACE_COUNT)
      {
