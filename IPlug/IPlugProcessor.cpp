...
void IPlugProcessor::ProcessMidiMsg(const IMidiMsg& msg) { SendMidiMsg(msg); }
...
void IPlugProcessor::ProcessBuffers(PLUG_SAMPLE_DST type, int nFrames) { ProcessBlock(mScratchData[ERoute::kInput].Get(), mScratchData[ERoute::kOutput].Get(), nFrames); }
