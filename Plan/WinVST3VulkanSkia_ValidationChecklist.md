...
- [ ] Launch each host at 150 % scaling and open the plug-in editor. Confirm the `SetHostContentScaleBypassed` log flips to `true`, indicating the thread entered the per-monitor-aware context, the paired debug logs report a physical scale ≈1.5 while the host DPI scale remains ≈1.0, and that `PlatformResize` shows `window`/`target` ≈ host with `physicalScale` ≈ render scale and a virtualization ratio ≈ physical ÷ host. Cross-check the `DrawResize` `DBGMSG` to ensure the render target dimensions equal logical size × physical scale.【F:IGraphics/Platforms/IGraphicsWin.cpp†L3554-L3634】【F:IGraphics/Platforms/IGraphicsWin.cpp†L4346-L4416】【F:IGraphics/Drawing/IGraphicsSkia.cpp†L1316-L1478】
