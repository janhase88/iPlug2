...
3. **External Research** ✅
   - Review Vulkan specification sections referenced by validation layers (synchronization, image layout transitions, memory alignment).
   - Consult Skia Vulkan backend integration notes on `VulkanExtensions` and backend-context population.
...
5. **Implement Fixes** ✅
   ...
   - Propagate physical-device features/properties (including `VkPhysicalDeviceFeatures2` and memory limits) to Skia so flush alignment and layout decisions match the hardware capabilities, while guarding the propagation logic for older Skia SDKs.
