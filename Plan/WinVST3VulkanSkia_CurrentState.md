...
- The helper now tracks `mWindowDPIScale` (host logical DPI) separately from `mPhysicalWindowScale`, keeping host callbacks stable while letting window sizing and rendering target the measured pixels during bypass mode.【F:IGraphics/Platforms/IGraphicsWin.cpp†L4381-L4416】【F:IGraphics/Platforms/IGraphicsWin.h†L91-L109】

## Window Resizing and DPI Events
- `PlatformResize()` reports the logical window scale, host scale, physical render dimensions, and the current HWND size before adjusting anything, then logs the final size so we can confirm the host frame stayed at the logical bounds while the renderer targeted the physical pixels. The virtualization ratio (`render/host`) is included for quick sanity checks.【F:IGraphics/Platforms/IGraphicsWin.cpp†L3554-L3771】【F:IGraphics/Platforms/IGraphicsWin.h†L91-L106】
...
- `CreateVulkanContext()` pulls `vkGetPhysicalDeviceSurfaceCapabilitiesKHR`, then precomputes the logical width/height multiplied by the measured physical DPI whenever the bypass is active before delegating to `CreateOrResizeVulkanSwapchain()`, ensuring the very first swapchain targets the monitor pixels instead of the host's virtualized extent. A paired `DBGMSG` (`SwapchainExtent`) records the chosen extent and whether virtualization was bypassed to simplify log review.【F:IGraphics/Platforms/IGraphicsWin.cpp†L3971-L4250】
...
- Every platform-scale refresh now emits both a `DBGMSG` and an `IGRAPHICS_VK_LOG` info event with the measured physical scale, host DPI scale, bypass state, and whether the refresh was triggered manually. Logging stays enabled even in release builds by forcing `DBGMSG` on and setting the Vulkan logger verbosity to verbose.【F:IGraphics/Platforms/IGraphicsWin.cpp†L47-L62】【F:IGraphics/Platforms/IGraphicsWin.cpp†L4381-L4416】
...
- `SetHostContentScaleBypassed()` logs its state transition (including whether mixed-DPI hosting was successfully enabled) and records a matching Vulkan event, giving immediate confirmation that the bypass engaged before the first scale refresh.【F:IGraphics/Platforms/IGraphicsWin.cpp†L4346-L4368】
