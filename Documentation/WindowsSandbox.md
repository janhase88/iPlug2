...
## Purpose
The optional sandbox configuration in iPlug2 allows Windows plug-ins to behave as if each instance is hosted in isolation. When enabled, the macros in `IPlug/Sandbox/IPlugSandboxConfig.h` ensure that shared globals—such as the module handle, Win32 window classes, font caches, Skia factories, and Vulkan logging sinks—are no longer shared across instances. This prevents state leaks between plug-ins that would otherwise reuse these resources inside the same process.
...
2. Include iPlug's umbrella headers (`IPlug_include_in_plug_src.h`, `IGraphics_include_in_plug_src.h`) as usual. They already include `IPlug/Sandbox/IPlugSandboxConfig.h`, so no additional includes are required.
...
- **Shared state persists:** Verify the relevant module toggle is enabled and that the translation unit includes `IPlug/Sandbox/IPlugSandboxConfig.h` before any conditional compilation occurs.
