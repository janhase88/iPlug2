...
bool IPlugAPP::EditorResize(int viewWidth, int viewHeight)
{
  ...
  if (viewWidth != GetEditorWidth() || viewHeight != GetEditorHeight())
  {
    ...
    SetWindowPos(hwnd, 0, static_cast<LONG>(rcWindow.left * ss), static_cast<LONG>((rcWindow.bottom - viewHeight - ptDiff.y) * ss), static_cast<LONG>((viewWidth + ptDiff.x) * ss),
...
void IPlugAPP::SendSysexMsgFromUI(const ISysEx& msg) { SendSysEx(msg); }
