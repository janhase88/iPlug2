 ...
 *
 *  This is the lowest level base class in iPlug 2 that facilitates distributing editor and DSP parts for plug-in formats that need that, but also allowing non-distributed plug-ins to use the same
 * API. In distributed plug-in architectures certain methods will be overridden in order to pipe messages to various places, using whatever mechanism that plug-in format requires. In this case, there
 * are actually two classes that implement the IEditorDelegate interface, but only one which is directly connected to the user interface (IGraphics etc.), the other being connected to a class
 * inheriting IPlugAPIBase that deals with processing audio, see for example IPlugVST3Processor
 *
...
#pragma mark - Methods you may want to override...
   ...
   * Like OnParamChange, OnParamChangeUI will be called when a parameter changes. However, whereas OnParamChange may be called on the audio thread and should be used to update DSP state,
   * OnParamChangeUI is always called on the low-priority thread, should be used to update UI (e.g. for hiding or showing controls). You should not update parameter objects using this method.
   * @param paramIdx The index of the parameter that changed */
   ...
   * Override only if you need to handle notifications and updates in a specialist manner (e.g. if the ordering of updating parameters has an effect or if you need to avoid multiple settings of linked
   * parameters). This must update both DSP and UI. The default implementation calls OnParamChange() and OnParamChangeUI() for each parameter.
...
private:
  /** A list of IParam objects. This list is populated in the delegate constructor depending on the number of parameters passed as an argument to MakeConfig() in the plug-in class implementation
   * constructor */
