...
- `PlatformResize()` now reports the logical window scale, host scale, target pixel dimensions, render pixel dimensions, and the current HWND size before resizing, then logs the final size after applying the resize so validation can confirm the parent chain reached the physical pixel target (`targetPixels` ≈ `renderPixels`).【F:IGraphics/Platforms/IGraphicsWin.cpp†L3554-L3771】【F:IGraphics/Platforms/IGraphicsWin.h†L91-L106】
