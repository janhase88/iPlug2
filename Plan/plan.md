...
- Split the host window scale from the physical render scale so `GetPlatformWindowScale()` reports the physical pixels while host callbacks continue to use the logical DPI for negotiations.
...
- [ ] Decouple host window DPI from the physical render DPI so window sizing honours the measured pixels without confusing host resize negotiations.
