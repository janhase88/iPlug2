...
#if !defined(NDEBUG) || defined(IGRAPHICS_DEBUG_RESOURCE_LOAD)
void IGraphics::EnableResourceLoadProfiling(bool enable) { sResourceLoadProfiling = enable; }

bool IGraphics::IsResourceLoadProfilingEnabled() { return sResourceLoadProfiling; }
#endif
...
void IGraphics::SetLayoutOnResize(bool layoutOnResize) { mLayoutOnResize = layoutOnResize; }
...
void IGraphics::RemoveControl(int idx) { RemoveControl(GetControl(idx)); }
...
IControl* IGraphics::AttachControl(IControl* pControl, int ctrlTag, const char* group, int zIndex)
{
  ...
  if (plug)
  {
    TRACE_SCOPE_F(plug->GetLogFile(), "AttachControl");
    Trace(plug->GetLogFile(), TRACELOC, "ctrlTag=%d group=%s", ctrlTag, group ? group : "");
  }
  if (ctrlTag > kNoTag)
  {
...
void IGraphics::RemovePopupMenuControl() { mPopupControl = nullptr; }
...
void IGraphics::RemoveTextEntryControl() { mTextEntryControl = nullptr; }
...
void IGraphics::HideControl(int paramIdx, bool hide) { ForMatchingControls(&IControl::Hide, paramIdx, hide); }

void IGraphics::DisableControl(int paramIdx, bool disable) { ForMatchingControls(&IControl::SetDisabled, paramIdx, disable); }
...
void IGraphics::SetAllControlsDirty() { ForAllControls(&IControl::SetDirty, false, -1); }

void IGraphics::SetAllControlsClean() { ForAllControls(&IControl::SetClean); }
...
void IGraphics::DrawVerticalLine(const IColor& color, float xi, float yLo, float yHi, const IBlend* pBlend, float thickness) { DrawLine(color, xi, yLo, xi, yHi, pBlend, thickness); }

void IGraphics::DrawHorizontalLine(const IColor& color, float yi, float xLo, float xHi, const IBlend* pBlend, float thickness) { DrawLine(color, xLo, yi, xHi, yi, pBlend, thickness); }
...
void IGraphics::SetPTParameterHighlight(int paramIdx, bool isHighlighted, int color) { ForMatchingControls(&IControl::SetPTParameterHighlight, paramIdx, isHighlighted, color); }
...
void IGraphics::PopupHostContextMenuForParam(int controlIdx, int paramIdx, float x, float y) { PopupHostContextMenuForParam(GetControl(controlIdx), paramIdx, x, y); }
...
WDL_TypedBuf<uint8_t> IGraphics::LoadResource(const char* fileNameOrResID, const char* fileType)
{
  ...
  auto* plug = dynamic_cast<IPluginBase*>(GetDelegate());
  if (plug)
    Trace(plug->GetLogFile(), TRACELOC, "LoadResource start name:%s type:%s", fileNameOrResID, fileType);
  ...
  if (plug)
    Trace(plug->GetLogFile(), TRACELOC, "LoadResource resolved name:%s location:%d path:%s", fileNameOrResID, (int)resourceFound, path.Get());
  ...
  if (resourceFound == EResourceLocation::kNotFound)
  {
    ...
    if (plug)
      Trace(plug->GetLogFile(), TRACELOC, "LoadResource end name:%s not_found %lldus", fileNameOrResID, (long long)us);
    return result;
  }
  ...
  if (resourceFound == EResourceLocation::kAbsolutePath)
  {
    ...
    if (!fd)
    {
      auto us = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - start).count();
      if (plug)
        Trace(plug->GetLogFile(), TRACELOC, "LoadResource end name:%s fopen_failed %lldus", fileNameOrResID, (long long)us);
      return result;
    }
    ...
    if (fseek(fd, 0, SEEK_END))
    {
      ...
      auto us = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - start).count();
      if (plug)
        Trace(plug->GetLogFile(), TRACELOC, "LoadResource end name:%s seek_end_failed %lldus", fileNameOrResID, (long long)us);
      return result;
    }
    long size = ftell(fd);
    ...
    if (fseek(fd, 0, SEEK_SET))
    {
      ...
      auto us = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - start).count();
      if (plug)
        Trace(plug->GetLogFile(), TRACELOC, "LoadResource end name:%s seek_set_failed %lldus", fileNameOrResID, (long long)us);
      return result;
    }
    ...
    if (bytesRead != (size_t)size)
    {
      ...
      auto us = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - start).count();
      if (plug)
        Trace(plug->GetLogFile(), TRACELOC, "LoadResource end name:%s read_failed %lldus", fileNameOrResID, (long long)us);
      return result;
    }
    fclose(fd);
  }
  ...
  auto us = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - start).count();
  if (plug)
    Trace(plug->GetLogFile(), TRACELOC, "LoadResource end name:%s ok %lldus", fileNameOrResID, (long long)us);
...
IBitmap IGraphics::LoadBitmap(const char* name, int nStates, bool framesAreHorizontal, int targetScale)
{
  ...
  auto* plug = dynamic_cast<IPluginBase*>(GetDelegate());
  if (plug)
    Trace(plug->GetLogFile(), TRACELOC, "LoadBitmap start name:%s target:%d", name, targetScale);
  ...
  if (!pAPIBitmap)
  {
    ...
    if (!bitmapTypeSupported)
    {
      auto us = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - start).count();
      if (plug)
        Trace(plug->GetLogFile(), TRACELOC, "LoadBitmap end name:%s unsupported_ext %lldus", name, (long long)us);
    ...
    if (plug)
      Trace(plug->GetLogFile(), TRACELOC, "LoadBitmap resource search name:%s location:%d path:%s sourceScale:%d", name, (int)resourceLocation, fullPath.Get(), sourceScale);

    if (resourceLocation == EResourceLocation::kNotFound)
    {
      if (plug)
        Trace(plug->GetLogFile(), TRACELOC, "LoadBitmap fallback: searching cache for %s", name);
      ...
      if (plug)
        Trace(plug->GetLogFile(), TRACELOC, "LoadBitmap cache search result name:%s hit:%d sourceScale:%d", name, pAPIBitmap != nullptr, sourceScale);
    }
    else
    {
      ...
      {
        if (plug)
          Trace(plug->GetLogFile(), TRACELOC, "LoadBitmap loading resource %s", fullPath.Get());
    ...
    {
      if (plug)
        Trace(plug->GetLogFile(), TRACELOC, "LoadBitmap scaling from %d to %d", pAPIBitmap->GetScale(), targetScale);
      auto us = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - start).count();
      if (plug)
        Trace(plug->GetLogFile(), TRACELOC, "LoadBitmap end name:%s scaled %lldus", name, (long long)us);
    ...
    else if (loadedBitmap)
    {
      ...
      if (plug)
        Trace(plug->GetLogFile(), TRACELOC, "LoadBitmap retained new bitmap %s", name);
    }
  }

  auto us = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - start).count();
  if (plug)
    Trace(plug->GetLogFile(), TRACELOC, "LoadBitmap end name:%s target:%d %lldus", name, targetScale, (long long)us);
...
IBitmap IGraphics::LoadBitmap(const char* name, const void* pData, int dataSize, int nStates, bool framesAreHorizontal, int targetScale)
{
  ...
  auto* plug = dynamic_cast<IPluginBase*>(GetDelegate());
  if (plug)
    Trace(plug->GetLogFile(), TRACELOC, "LoadBitmap(data) start name:%s target:%d", name, targetScale);
  ...
  if (!pAPIBitmap)
  {
    ...
    if (!bitmapTypeSupported)
    {
      auto us = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - start).count();
      if (plug)
        Trace(plug->GetLogFile(), TRACELOC, "LoadBitmap(data) end name:%s unsupported_ext %lldus", name, (long long)us);
      return IBitmap(); // return invalid IBitmap
    }
    ...
    pAPIBitmap = SearchBitmapInCache(name, targetScale, sourceScale);
    if (plug)
      Trace(plug->GetLogFile(), TRACELOC, "LoadBitmap(data) cache search result name:%s hit:%d sourceScale:%d", name, pAPIBitmap != nullptr, sourceScale);

    // It's definitely not loaded, so load it with scale = 1.
    if (!pAPIBitmap)
    {
      if (plug)
        Trace(plug->GetLogFile(), TRACELOC, "LoadBitmap(data) loading from data");
    ...
    {
      if (plug)
        Trace(plug->GetLogFile(), TRACELOC, "LoadBitmap(data) scaling from %d to %d", pAPIBitmap->GetScale(), targetScale);
      auto us = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - start).count();
      if (plug)
        Trace(plug->GetLogFile(), TRACELOC, "LoadBitmap(data) end name:%s scaled %lldus", name, (long long)us);
      return ScaleBitmap(IBitmap(pAPIBitmap, nStates, framesAreHorizontal, name), name, targetScale);
    }
    else if (loadedBitmap)
    {
      RetainBitmap(IBitmap(loadedBitmap.release(), nStates, framesAreHorizontal, name), name);
      if (plug)
        Trace(plug->GetLogFile(), TRACELOC, "LoadBitmap(data) retained new bitmap %s", name);
    }
  }

  auto us = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - start).count();
  if (plug)
    Trace(plug->GetLogFile(), TRACELOC, "LoadBitmap(data) end name:%s target:%d %lldus", name, targetScale, (long long)us);
...
EResourceLocation IGraphics::SearchImageResource(const char* name, const char* type, WDL_String& result, int targetScale, int& sourceScale)
{
  ...
  auto* plug = dynamic_cast<IPluginBase*>(GetDelegate());
  if (plug)
    Trace(plug->GetLogFile(), TRACELOC, "SearchImageResource start name:%s type:%s target:%d", name, type, targetScale);
  ...
  for (sourceScale = targetScale; sourceScale > 0; SearchNextScale(sourceScale, targetScale))
  {
    ...
    if (found > EResourceLocation::kNotFound)
    {
      auto us = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - start).count();
      if (plug)
        Trace(plug->GetLogFile(), TRACELOC, "SearchImageResource end name:%s location:%d path:%s sourceScale:%d %lldus", name, (int)found, result.Get(), sourceScale, (long long)us);
      return found;
    }
  }
  ...
  auto us = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - start).count();
  if (plug)
    Trace(plug->GetLogFile(), TRACELOC, "SearchImageResource end name:%s not_found %lldus", name, (long long)us);
...
APIBitmap* IGraphics::SearchBitmapInCache(const char* name, int targetScale, int& sourceScale)
{
  ...
  for (sourceScale = targetScale; sourceScale > 0; SearchNextScale(sourceScale, targetScale))
  {
    ...
    if (pBitmap)
    {
      auto us = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - start).count();
      if (plug)
        Trace(plug->GetLogFile(), TRACELOC, "SearchBitmapInCache end name:%s hit scale:%d %lldus", name, sourceScale, (long long)us);
      return pBitmap;
    }
  }
  ...
  auto us = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - start).count();
  if (plug)
    Trace(plug->GetLogFile(), TRACELOC, "SearchBitmapInCache end name:%s miss %lldus", name, (long long)us);
  return nullptr;
}
...
void IGraphics::CreatePopupMenu(IControl& control, IPopupMenu& menu, const IRECT& bounds, int valIdx) { DoCreatePopupMenu(control, menu, bounds, valIdx, false); }
...
void IGraphics::DrawPoint(const IColor& color, float x, float y, const IBlend* pBlend) { FillRect(color, IRECT(x, y, x + 1.f, y + 1.f), pBlend); }
...
void IGraphics::PathRoundRect(const IRECT& bounds, float cr) { PathRoundRect(bounds, cr, cr, cr, cr); }
...
void IGraphics::PathEllipse(const IRECT& bounds) { PathEllipse(bounds.MW(), bounds.MH(), bounds.W() / 2.f, bounds.H() / 2.f); }
...
void IGraphics::PathTransformSave() { mTransformStates.push(mTransform); }
...
void IGraphics::PathTransformScale(float scale) { PathTransformScale(scale, scale); }
...
IPattern IGraphics::GetSVGPattern(const NSVGpaint& paint, float opacity)
{
  ...
  case NSVG_PAINT_RADIAL_GRADIENT: {
    ...
    case NSVG_SPREAD_PAD:
      pattern.mExtend = EPatternExtend::Pad;
      break;
    case NSVG_SPREAD_REFLECT:
      pattern.mExtend = EPatternExtend::Reflect;
      break;
    case NSVG_SPREAD_REPEAT:
      pattern.mExtend = EPatternExtend::Repeat;
      break;
    }
...
void IGraphics::DoDrawSVG(const ISVG& svg, const IBlend* pBlend, const IColor* pStrokeColor, const IColor* pFillColor)
{
  ...
  for (NSVGshape* pShape = pImage->shapes; pShape; pShape = pShape->next)
  {
    ...
    if (pShape->stroke.type != NSVG_PAINT_NONE)
    {
      ...
      {
      case NSVG_CAP_BUTT:
        options.mCapOption = ELineCap::Butt;
        break;
      case NSVG_CAP_ROUND:
        options.mCapOption = ELineCap::Round;
        break;
      case NSVG_CAP_SQUARE:
        options.mCapOption = ELineCap::Square;
        break;
      }
      ...
      case NSVG_JOIN_MITER:
        options.mJoinOption = ELineJoin::Miter;
        break;
      case NSVG_JOIN_ROUND:
        options.mJoinOption = ELineJoin::Round;
        break;
      case NSVG_JOIN_BEVEL:
        options.mJoinOption = ELineJoin::Bevel;
        break;
      }
