...
| `DBGMSG` / `IGRAPHICS_VK_LOG` (event: `PlatformResize`) | Logged on each platform resize, showing the window scale, host scale, target pixels, render pixels, current size, bypass flag, virtualization ratio, and whether the parent already resized; a follow-up log records the applied size.【F:IGraphics/Platforms/IGraphicsWin.cpp†L3554-L3771】 | Use to confirm the HWND chain grows to the physical pixel size (target ≈ render) and investigate any mismatch between requested and final dimensions. |
